...
# load the data
#tut <- read_dta("TutorialData.dta")
add <-  read.csv("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Working_projects/Tap.water/SharedDSRB/Data/TUKsocioeconomics_from_descriptives_2021_eth_edu_imd.csv")## data as in 4.Heritability_Of_IngR.RMD
add$X <- NULL
tut <- merge(df, add, by.x="study_no", by.y="sn")
...
tut$Ethnic_Origin <- ifelse(is.na(tut$ethnicity_cat_combined), NA, ifelse(tut$ethnicity_cat_combined== "White", 1, 2))
tut$SEX <- ifelse(is.na(tut$SEX), NA, ifelse(tut$SEX=="F",1,2))
tut$Agein2022.gr <- ifelse(is.na(tut$Agein2022), NA, 
                           ifelse(tut$Agein2022.gr == "1.Under60", 1, ifelse(tut$Agein2022.gr == "2.60-71", 2,3)))
tut$Employ_bin <- ifelse(is.na(tut$Employ_bin), NA, ifelse(tut$Employ_bin=="NotRetired",1,2))
tut$edu_bin_combined <- ifelse(is.na(tut$edu_bin_combined), NA, ifelse(tut$edu_bin_combined=="All else",1,2))

# Generate the stratum ID
tut$stratum <-  10000*tut$SEX + 1000*tut$Ethnic_Origin + 100*tut$Employ_bin + 
                10*tut$edu_bin_combined + 1*tut$Agein2022.gr
...
tutx <- subset(tut, strataN > 5)
##############################################
# Estimate linear model 1A
##############################################

tutx <- subset(tutx, !is.na(stratum))
tutx <- subset(tutx, !is.na(total.consump.L))

# Fit the two-level linear regression with no covariates
model1A <- lmer(total.consump.L ~ (1|stratum), data=tutx)
summary(model1A)
...
stratum_level <- aggregate(x=tut2[c("total.consump.L")], 
                  by=tut2[c("SEX", "Ethnic_Origin", "Employ_bin", "edu_bin_combined", "Agein2022.gr",
                  "stratum", "strataN", "m1Am", "m1Bmfit", "m1Bmupr", "m1Bmlwr")],
                  FUN=mean)

total.consump.L ~ SEX + Ethnic_Origin + Employ_bin + edu_bin_combined + Agein2022.gr
...
tut$Ethnic_Origin <- ifelse(is.na(tut$Ethnic_Origin), NA, ifelse(tut$Ethnic_Origin == "White", "White", "Raciialised"))

# Generate the stratum ID
tut$stratum <- 10000*tut$SEX + 1000*tut$Ethnic_Origin + 100*tut$e + 
